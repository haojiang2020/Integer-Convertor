...
def integer_mod_inverse(x, m, radix, check = True):
    ...
    if not res is None:
        ...
        x = integer_modulo(x, m, radix, check = False)["rem"]
        temp_bool = True
        temp_vec_0 = x
        while temp_bool:
...
def integer_pm_seq_complete_root_condition(c, b, p_vec, radix, inverse = True):
    ...
    if res:
        ...
        if integer_is_relatively_primes(c, m, radix, check = False):
            if integer_is_relatively_primes(b, phi_m, radix, check = False):
                if inverse:
                    inv_c = integer_mod_inverse(c, m, radix, check = False)
                    inv_b = integer_mod_inverse(b, phi_m, radix, check = False)
            else:
                res = False
        else:
            res = False
...
def integer_pm_seq_index(x, pm_seq_cr_dict, radix):
    ...
    if integer_check(x, radix) & isinstance(pm_seq_cr_dict, dict):
        ...
        if (("type" in temp_str_list) & 
            ...
            res = True
    if res:
        res = power_modulo(x, pm_seq_cr_dict["mod_inverse_b"], 
                           pm_seq_cr_dict["m"], 
                           radix, check = False)
        res = multiply_modulo(res, pm_seq_cr_dict["mod_inverse_c"], 
                              pm_seq_cr_dict["m"], 
                              radix, check = False)
    else:
        res = None
