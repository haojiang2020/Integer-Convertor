...
def integer_check(x, radix):
    ...
    # x = x[1]*(radix^1) + x[2]*(radix^2) + ... + x[x_len-1]*(radix^(x_len-1))
    res = False
...
def check_less_tuple_tuple(x, y, radix, check = True):
    if check:
...
def check_lesseq_tuple_tuple(x, y, radix, check = True):
    if check:
        if integer_check(x, radix) & integer_check(y, radix): 
            res = True
        else:
...
def check_more_tuple_tuple(x, y, radix, check = True):
    if check:
        if integer_check(x, radix) & integer_check(y, radix): 
            res = True
        else:
...
def check_moreeq_tuple_tuple(x, y, radix, check = True):
    if check:
        if integer_check(x, radix) & integer_check(y, radix): 
            res = True
        else:
...
def check_eq_tuple_tuple(x, y, radix, check = True):
    if check:
        if integer_check(x, radix) & integer_check(y, radix): 
            res = True
        else:
...
def check_noteq_tuple_tuple(x, y, radix, check = True):    
    if check:
        if integer_check(x, radix) & integer_check(y, radix): 
            res = True
        else:
...
def integer_plus(x, y, radix, check = True):
    ...
    if check:
        ...
        res = True
    if not res is None:         
        if check_moreeq_tuple_tuple(integer_absolute(x, radix, check), 
...
def integer_modulo(x, y, radix, check = True):
    # input: integer vector, x, y
    #                        where |y| > 1
    # output: integer vector, {"quo": q, "rem": r}
    #                         where r+qy = x
    if check:
        res = None
        if integer_check(x, radix) & integer_check(y, radix): 
    ...
    if not res is None:
        ...
        if check_less_tuple_tuple(x, y, radix, check = False):
            temp_vec_0 = x
            temp_vec_1 = [True, 0]
        else:
            x_len = len(x)
            ...
            while temp_bool_0:
                ...
                temp_mid_num_vec = integer_multiply(temp_mid_vec, 
                                                    y, radix, 
                                                    check = False)
                if check_less_tuple_tuple(x, temp_mid_num_vec, 
                                          radix, check = False):
                    temp_upper_vec = temp_mid_vec
                elif check_more_tuple_tuple(x, temp_mid_num_vec, 
                                            radix, check = False):
                ...
                if temp_bool_1:
                    temp_diff_vec = integer_minus(temp_upper_vec, 
                                                  temp_lower_vec, 
                                                  radix, check = False)
                    if len(temp_diff_vec) == 2:
                        if temp_diff_vec[1] <= 1:
                            temp_bool_0 = False
                else:
                    temp_bool_0 = False
            if temp_bool_1:
...
def integer_power(x, y, radix, check = True):
    ...
    if check:
        res = None
        if integer_check(x, radix) & integer_check(y, radix): 
            if check_moreeq_tuple_tuple(y, [True, 0], radix, check = False):
                res = True 
    else:
        res = True
    if not res is None:        
        if check_more_tuple_tuple(y, [True, 1], radix, check = False):
            if check_eq_tuple_tuple(x, [True, 0], radix, check = False):
                res = tuple([True, 0])
            elif check_eq_tuple_tuple(x, [True, 1], radix, check = False):
                res = tuple([True, 1])
            elif check_eq_tuple_tuple(x, [False, 1], radix, check = False):
                temp_bool = True
                temp_sign = True
                temp_num_vec = [True, 0]
                while temp_bool:
                    temp_sign = not temp_sign
                    if check_moreeq_tuple_tuple(temp_num_vec, y, radix, check = False):
                        temp_bool = False
                ...
                for n in range(1, y_len):
                    ...
                    if n < y_len-1:
                        temp_num_vec_0 = integer_multiply(temp_num_vec_0, temp_num_vec_0, 
                                                          radix, check = False)
        elif check_more_tuple_tuple(y, [True, 0], radix, check = False):
            res = tuple(x)
        else:
            res = tuple([True, 1])
    return res
...
def integer_factorial(x, radix, check = True):
    ...
    if check:
        res = None
        if integer_check(x, radix): 
            if check_moreeq_tuple_tuple(x, [True, 0], radix, check = False):
                res = True 
    else:
        res = True
    if not res is None: 
        if check_more_tuple_tuple(x, [True, 1], radix, check = False):
            res = [True, 1]
            temp_bool = True
            temp_num = [True, 1]
            while temp_bool:
                ...
                res = integer_multiply(res, temp_num, 
                                       radix, check = False)
                if check_moreeq_tuple_tuple(temp_num, x, 
                                            radix, check = False):
                    temp_bool = False
        else:
            res = tuple([True, 1])
...
def integer_permute(m, n, radix, check = True):
    ...
    if check:
        ...
        if integer_check(m, radix) & integer_check(n, radix): 
            if check_more_tuple_tuple(n, [True, 0], radix, check = False):
                if check_moreeq_tuple_tuple(m, n, radix, check = False):
                    res = True 
    else:
        res = True
    if not res is None:
        ...
        while temp_bool:
            ...
            if check_lesseq_tuple_tuple(n, [True, 0], radix, check = False):
                temp_bool = False
    return res
...
def integer_choose(m, n, radix, check = True):
    ...
    if check:
        res = None
        if integer_check(m, radix) & integer_check(n, radix): 
            if check_moreeq_tuple_tuple(n, [True, 0], radix, check = False):
                if check_moreeq_tuple_tuple(m, n, radix, check = False):
                    if check_more_tuple_tuple(m, [True, 0], radix, check = False):
                        res = True 
    else:
        res = True
    if not res is None:
        if check_eq_tuple_tuple(n, [True, 0], radix, check = False):
            res = tuple([True, 1])
        elif check_eq_tuple_tuple(n, m, radix, check = False):
            res = tuple([True, 1])
        else:
            temp_vec_0 = integer_minus(m, n, radix, check = False)
            if check_lesseq_tuple_tuple(temp_vec_0, n, radix, check = False):
                n = temp_vec_0
            temp_vec_0 = [True, 1]
            temp_vec_1 = [True, 1]
            temp_bool = True
            while temp_bool:
                temp_vec_0 = integer_multiply(temp_vec_0, m, radix, check = False)
                m = minus_one(m, radix, check = False)
                temp_vec_1 = integer_multiply(temp_vec_1, n, radix, check = False)
                n = minus_one(n, radix, check = False)
                if check_lesseq_tuple_tuple(n, [True, 0], radix, check = False):
                    temp_bool = False
            res = integer_modulo(temp_vec_0, temp_vec_1, 
                                 radix, check = False)["quo"]
    return res

def integer_modular_inverse(x, m, radix, check = True):
    ...
    if check:
        ...
        if integer_check(x, radix) & integer_check(m, radix): 
            if (check_less_tuple_tuple(m, [False, 1], radix, check = False) | 
                check_more_tuple_tuple(m, [True, 1], radix, check = False)):                
                    if check_noteq_tuple_tuple(x, [True, 0], radix, check = False):  
                        temp_vec = integer_gcd(x, m, radix, check = False)
                        if (len(temp_vec) == 2) & (temp_vec[1] < 2):
                            res = True 
    else:
        res = True
    if not res is None:
        ...
        temp_vec_0 = x
        while temp_bool:
            temp_vec_1 = multiply_modulo(temp_vec_0, x, m, radix, check = False)
            if (len(temp_vec_1) == 2) & (temp_vec_1[1] < 2):
                res = temp_vec_0
                temp_bool = False
            else:
                temp_vec_0 = temp_vec_1
    return res

def integer_crt(r, m, radix, check = True):
    # input: vector of integer vector, r, m
    #                          where r is the vector ofobjective remainders
    #                          where m is the vector of divisors
    #                                & all the elements in m are pairwise mutually primes 
    #                                  (excluding -1, 0, 1)
                ...
                if (r_len == m_len) & (r_len > 1):
                    ...
                    for n in range(r_len):
                        if integer_check(r[n], radix) & integer_check(m[n], radix): 
                            if (check_less_tuple_tuple(m[n], [False, 1], 
                                                       radix, check = False) |
                                check_more_tuple_tuple(m[n], [True, 1], 
                                                       radix, check = False)):
                                for n1 in range(n):
                                    temp_vec = integer_gcd(m[n], m[n1], radix, 
                                                           check = False)
                                    if (len(temp_vec) != 2) | (temp_vec[1] != 1):
                                        temp_bool_0 = False
                                        break
                            else:
                                temp_bool_0 = False
    ...
    if not res is None:
        ...
        for n in range(r_len):
            temp_num = integer_modulo(M, m[n], 
                                      radix, check = False)["quo"]
            temp_num_1 = integer_modular_inverse(temp_num, m[n], 
                                                 radix, check = False)
        ...
        if check_more_tuple_tuple(x, 
            ...
            while temp_bool_0:
                ...
                if not True in temp_prime_list[3]:
                    if check_lesseq_tuple_tuple(temp_vec_0, 
                                                half_sqrt_num, 
                                                radix, check = False):
                        temp_prime_list[0] += 1
                    ...
                    if (len(temp_modulo_rem) == 2) & (temp_modulo_rem[1] < 1):
                        res = False
                if res:
                    if check_moreeq_tuple_tuple(temp_vec_0, 
                                                sqrt_num, 
                                                radix, check = False):
                        temp_bool_0 = False
                    else:
...
def integer_factorization(x, radix, check = True):
    ...
    if not res is None:  
        ...
        if check_more_tuple_tuple(x, 
            ...
            while temp_bool_0:
                ...
                if not True in temp_prime_list[3]:
                    ...
                    temp_bool_2 = True
                    while temp_bool_1:
                        if check_more_tuple_tuple(x, temp_vec_0, 
                                                  radix, check = False):
                ...
                if (len(x) == 2) & (x[1] < 2):
                    temp_bool_0 = False
                elif check_moreeq_tuple_tuple(temp_vec_0, 
                                              sqrt_num, 
                                              radix, check = False):
        ...
        elif check_moreeq_tuple_tuple(x, 
                                      integer_int_2_tuple(16, radix, check = False), 
                                      radix, check = False):
    ...
    if not res is None:
        ...
        if (len(x) == 2) & (x[1] < 1) & (len(y) == 2) & (y[1] < 1):
            ...
            if (len(x) == 2) & (x[1] < 2):
                ...
                if check_eq_tuple_tuple(x, y, radix, 
                                        check = False):
                    res = x
                    temp_bool_0 = False                
                elif check_more_tuple_tuple(x, y, radix, 
                                            check = False):
                    x_1 = x
                    y_1 = y
                    temp_bool_0 = True
...
def integer_am_seq_complete_root_condition(c, a, m, radix):
    ...
    if integer_check(c, radix) & integer_check(a, radix) & integer_check(m, radix): 
        m = integer_absolute(m, radix, check = False)
        if check_more_tuple_tuple(m, [True, 1], radix, 
                                  check = False):
            c = integer_modulo(c, m, radix, check = False)["rem"]    
            temp_vec = integer_gcd(c, m, radix, check = False)
            if (len(temp_vec) == 2) & (temp_vec[1] == 1):
                inv_c = integer_modular_inverse(c, m, radix, check = False)
...
def integer_t1gm_seq_primitive_root_condition(c, a, p, p_power, radix, double = False):
    ...
    if isinstance(double, bool):
        if (integer_check(c, radix) & integer_check(a, radix) & 
            ...
            if (check_more_tuple_tuple(p_power, [True, 0], radix, check = False) &
                check_noteq_tuple_tuple(c, [True, 0], radix, check = False) & 
                check_noteq_tuple_tuple(a, [True, 0], radix, check = False)):
                p = integer_absolute(p, radix, check = False)
                temp_2_vec = integer_int_2_tuple(2, radix, check = False)
                if check_eq_tuple_tuple(p, temp_2_vec, 
                                        radix, check = False):
                    temp_vec = integer_modulo(c, temp_2_vec, radix, check = False)["rem"]
                    if (len(temp_vec) == 2) & (temp_vec[1] < 1):
                        res = False
                    if res:
                        temp_vec = integer_modulo(a, temp_2_vec, radix, check = False)["rem"]
                        if (len(temp_vec) == 2) & (temp_vec[1] < 1):
                            res = False
                    if res:
                        if double:
                            if check_eq_tuple_tuple(p_power, [True, 1], 
                                                    radix, check = False):
                            ...
                            if check_eq_tuple_tuple(p_power, temp_2_vec, 
                                                    radix, check = False):
                                m = integer_int_2_tuple(4, radix, check = False)
                                factor_vec = tuple([tuple(temp_2_vec)])
                                factor_power_vec = tuple([tuple(temp_2_vec)])
                                phi_m = tuple(temp_2_vec)
                            elif check_eq_tuple_tuple(p_power, [True, 1], 
                                                      radix, check = False):
                                m = tuple(temp_2_vec)
                                factor_vec = tuple([tuple(temp_2_vec)])
                                ...
                                res = False
                elif check_more_tuple_tuple(p, temp_2_vec, 
                                            radix, check = False):
                    if integer_is_prime(p, radix, check = False):
                        temp_vec = integer_modulo(c, p, radix, check = False)["rem"]
                        if (len(temp_vec) == 2) & (temp_vec[1] < 1):
                            res = False
                        if res:
                            temp_vec = integer_modulo(a, p, radix, check = False)["rem"]
                            if (len(temp_vec) == 2) & (temp_vec[1] < 1):
                                res = False
                        if res:
                            if double:
                                ...
                                if res:
                                    ...
                                    if (len(temp_vec) == 2) & (temp_vec[1] < 1):
                                        res = False
                                if res:
                                    m = integer_power(p, p_power, radix, check = False)
                                    m = integer_multiply(m, temp_2_vec, radix, check = False)
                                    factor_vec = tuple([tuple(temp_2_vec), p])
    ...
    if res:
        c = integer_modulo(c, m, radix, check = False)["rem"]
        a = integer_modulo(a, m, radix, check = False)["rem"]
        if check_more_tuple_tuple(phi_m, [True, 1], 
                                  radix, check = False):            
            ...
            while temp_bool:
                ...
                if (len(temp_vec_1) == 2) & (temp_vec_1[1] < 2):
                    res = False
                    temp_bool = False
                elif check_moreeq_tuple_tuple(temp_vec_0, half_phi_m, 
                                              radix, check = False):
                    temp_bool = False
    if res:        
...
def integer_t1gm_seq_convert(x, t1pm_seq_pr_dict, radix):
    ...
    # input: integer vector, x
    #        condition tuple, t1pm_seq_pr_dict
    ...
    if res:
        ...
        if check_more_tuple_tuple(x, [True, 0], 
                                  radix, check = False):        
            temp_bool = True
            res = t1pm_seq_pr_dict["c"]
            temp_vec = [True, 0]
            while temp_bool:
                temp_vec = plus_one(temp_vec, radix, check = False)
                res = multiply_modulo(res, t1pm_seq_pr_dict["a"], 
                                      t1pm_seq_pr_dict["m"], radix, 
                                      check = False)
                if check_moreeq_tuple_tuple(temp_vec, x, radix, 
                                            check = False):
                    temp_bool = False
        else:
            res = t1pm_seq_pr_dict["c"]
...
def integer_t1gm_seq_index(x, t1pm_seq_pr_dict, radix):
    ...
    if integer_check(x, radix) & isinstance(t1pm_seq_pr_dict, dict):
        ...
        if ("type" in temp_str_list) & ("m" in temp_str_list):
            if t1pm_seq_pr_dict["type"] == "t1gm_seq_primitive_root":
                x = integer_modulo(x, t1pm_seq_pr_dict["m"], radix, check = False)["rem"]
                temp_vec = integer_gcd(x, t1pm_seq_pr_dict["m"], radix)
                if (len(temp_vec) == 2) & (temp_vec[1] < 2):
                    res = True
    if res:
        temp_bool = True
        ...
        while temp_bool:
            ...
            temp_vec_1 = multiply_modulo(temp_vec_1, t1pm_seq_pr_dict["a"], 
                                         t1pm_seq_pr_dict["m"], radix, 
                                         check = False)
            if check_eq_tuple_tuple(temp_vec_1, x, radix, 
                                    check = False):
                temp_bool = False
        res = temp_vec_0
    else:
...
def integer_pm_seq_complete_root_condition(c, b, p_vec, radix, inverse = True):
                    ...
                    if isinstance(p_vec[0], bool):
                        ...
                        for n in range(p_len):
                            if integer_check(p_vec[n], radix):
                                ...
                                if check_more_tuple_tuple(temp_vec, [True, 1], radix, 
                                                          check = False): 
                                    if integer_is_prime(temp_vec, radix, check = False):
                                        temp_bool_1 = True
                                        temp_num_0 = 0
                                        for n1 in range(n):
                                            if check_less_tuple_tuple(temp_vec, m_factor[n1], 
                                                                      radix, check = False):
                                                temp_bool_1 = False
                                                temp_num_0 = n1
                                                break
                                            elif check_eq_tuple_tuple(temp_vec, m_factor[n1], 
                                                                      radix, check = False):
                                                temp_bool_0 = False
                                                break
                                        if temp_bool_0:
                                            ...
                                            m = integer_multiply(m, temp_vec, 
                                                                 radix, check = False)
                                            phi_m = integer_multiply(phi_m, 
                                                                     minus_one(temp_vec, radix, check = False),
                                                                     radix, check = False)
                                    else:
                                        temp_bool_0 = False
                                else:
    ...
    if res:
        c = integer_modulo(c, m, radix, check = False)["rem"]
        b = integer_modulo(b, phi_m, radix, check = False)["rem"]
        temp_vec = integer_gcd(c, m, radix, check = False)
        if (len(temp_vec) == 2) & (temp_vec[1] == 1):
            temp_vec = integer_gcd(b, phi_m, radix, check = False)
            if (len(temp_vec) == 2) & (temp_vec[1] == 1):
                if inverse:
                    inv_c = integer_modular_inverse(c, m, radix, check = False)
                    inv_b = integer_modular_inverse(b, phi_m, radix, check = False)
            else:
                res = False
        else:
            res = False
...
def integer_pm_seq_convert(x, pm_seq_cr_dict, radix):
    ...
    # input: integer vector, x
    #        condition tuple, pm_seq_cr_dict
    ...
    if integer_check(x, radix) & isinstance(pm_seq_cr_dict, dict):
        ...
        if "type" in temp_str_list:
            if pm_seq_cr_dict["type"] == "pm_seq_complete_root":
                res = True
    if res:
        res = pm_seq_cr_dict["c"]
        temp_bool = True
        temp_vec = [True, 0]
        while temp_bool:
            temp_vec = plus_one(temp_vec, radix, check = False)
            res = multiply_modulo(res, x, pm_seq_cr_dict["m"], 
                                  radix, check = False)
            if check_moreeq_tuple_tuple(temp_vec, pm_seq_cr_dict["b"], 
                                        radix, check = False):
                temp_bool = False
    else:
        res = None
...
def integer_pm_seq_index(x, pm_seq_cr_dict, radix):
    ...
    if res:
        ...
        temp_vec_0 = [True, 0]
        res = [True, 1]
        temp_vec_1 = multiply_modulo(x, pm_seq_cr_dict["mod_inverse_c"], pm_seq_cr_dict["m"], 
                                     radix, check = False)
        while temp_bool:
            temp_vec_0 = plus_one(temp_vec_0, radix, check = False)
            res = multiply_modulo(res, temp_vec_1, pm_seq_cr_dict["m"], 
                                  radix, check = False)
            if check_moreeq_tuple_tuple(temp_vec_0, 
                                        pm_seq_cr_dict["mod_inverse_b"], 
                                        radix, check = False):
                temp_bool = False
    else:
        res = None
