...
integer_mod_inverse = function(x, m, radix, check = TRUE) {
  ...
  if (!is.null(res)) {
    ...
    x = integer_modulo(x, m, radix, check = FALSE)
    x = x[["rem"]]
    res = x
    temp_bool_1 = TRUE
    while (temp_bool_1) {
      temp_num = multiply_modulo(res, x, m, radix, check = FALSE)
      if ((length(temp_num) == 2) & (temp_num[2] < 2)) {
        temp_bool_1 = FALSE
      } else {
        res = temp_num
      }
  ...
  if (res) {
    ...
    for (n in 1:p_len) {
      ...
      m = integer_multiply(m, p_list[[temp_vec_1[n]]], 
                           radix, check = FALSE)
      phi_m = integer_multiply(phi_m, 
                               minus_one(p_list[[temp_vec_1[n]]], 
                                         radix, check = FALSE),  
                               radix, check = FALSE)
    }
    c = integer_modulo(c, m, radix, check = FALSE)
    c = c[["rem"]]
    b = integer_modulo(b, phi_m, radix, check = FALSE)
    b = b[["rem"]]
    res = FALSE
    if (integer_more_vec_vec(c, c(0, 0), radix, check = FALSE)) {
...
integer_pm_seq_index = function(x, pm_seq_cr_list, radix) {
  ...
  if (res) {
    res = power_modulo(x, pm_seq_cr_list[["mod_inverse_b"]], 
                       pm_seq_cr_list[["m"]], radix, check = FALSE)
    res = multiply_modulo(res, pm_seq_cr_list[["mod_inverse_c"]], 
                          pm_seq_cr_list[["m"]], radix, check = FALSE)
  } else {
    res = NULL
  }
